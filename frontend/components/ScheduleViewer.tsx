    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const eventsForDay = props.events.filter(
        (event) =>
          ...
          (moment(event.start).isBefore(date) &&
            moment(event.end).isAfter(date))
      );
  ...
  const renderWeekView = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push(
        <div
          key={`day-${i}`}
          className={`week-day ${isCurrentDate ? "current" : ""}`}
        >
          <div id="upper">
            {isSmallScreen ? date.format("dd") : date.format("ddd")}
          </div>
